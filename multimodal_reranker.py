...
from baseline import generate_adversarial_text_baseline, generate_adversarial_image_baseline
...
class MultimodalProductReranker:
    ...
    def _generate_adversarial_image(self, products, image_paths, target_product_idx, 
        ...
        for step in range(num_steps):
            ...
            if step > 0 and current_loss < best_loss:
    ...
    def test_multimodal_ranking(self, num_products=4, mode='baseline', target_product_idx=None):
        ...
        decoded_prompt = None
        if mode in ['attack_image', 'attack_text', 'attack_multimodal', 'attack_multimodal_baseline']:
        ...
        for i, product in enumerate(selected_products, 1):
            marker = "🎯" if mode in ['attack_image', 'attack_text', 'attack_multimodal', 'attack_multimodal_baseline'] and i-1 == target_product_idx else "  "
        ...
        if mode in ['attack_text', 'attack_image', 'attack_multimodal', 'attack_multimodal_baseline']:
            print(f"\n" + "="*60)
            ...
            if mode == 'attack_multimodal':
            ...
            elif mode == 'attack_multimodal_baseline':
                generated_text = generate_adversarial_text_baseline(
                    selected_products, 
                    target_product_idx
                )
                ...
                attack_products = [p.copy() for p in selected_products]
                original_desc = attack_products[target_product_idx].get('Description', 'No description available.')
                attack_products[target_product_idx]['Description'] = f"{original_desc}{generated_text}"
                
                baseline_image_path = generate_adversarial_image_baseline(
                    image_paths[target_product_idx]
                )
                
                updated_image_paths = image_paths.copy()
                updated_image_paths[target_product_idx] = baseline_image_path
                
                print(f"\n{'='*60}")
                print(f"🏁 FINAL TEST (After Baseline Multimodal Attack)")
                print(f"{'='*60}")
...
def main():
    ...
    parser.add_argument('--mode', type=str, default='baseline',
                       choices=['baseline', 'attack_image', 'attack_text', 'attack_multimodal', 'attack_multimodal_baseline'],
